...
def procesa_entregas(tank_id, volumen, volumen_ct, temperatura):
    ...
    try:
        ...
        if vol_dif > 50 and ini_entrega[0] == 'False':
            ...
            query = f"""UPDATE public."Tanques_tanques" set inicia_entrega = 'True', vol_ref = \'{"{:.2f}".format(float(vol_ref))}\', fecha_ref = \'{fecha}\', vol_ct_ref = \'{"{:.2f}".format(volumen_ct)}\', agua_ini = \'{"{:.2f}".format(float(vol_ant[4]))}\', temp_ini = \'{"{:.2f}".format(float(vol_ant[5]))}\', vol_ini = \'{"{:.2f}".format(float(vol_ant[2]))}\' WHERE vr_tanque = \'{vol_act[0]}\'"""
            cur.execute(query)
            conn.commit()
