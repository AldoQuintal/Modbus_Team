    ...
    try:
        ...
        for row in rows:
            ...
            try:
                ...
                print(f'vol_ante except: {vol_ante}')
            ...
            if inventario:
                ...
                query = f"""INSERT INTO inventarios (vr_tanque, vr_fecha, vr_volumen, vr_vol_ct, vr_agua, vr_temp, vol_ant) VALUES('{tank_key}', '{fecha}', '{"{:.2f}".format(val_vol)}', '{"{:.2f}".format(val_tc)}', '{"{:.2f}".format(val_agua)}', '{"{:.2f}".format(val_temp)}', '0')"""
                print(query)
                cur.execute(query)
                conn.commit()
...
def procesa_entregas(tank_id, volumen, volumen_ct, temperatura):
    ...
    try:
        ...
        if vol_dif > 50 and ini_entrega[0] == 'False':
            ...
            print(f'Vol_ant: {vol_ant}')
            vol_ref = vol_ant[2]
            now = datetime.now()
            fecha = now.strftime("%Y/%m/%d %H:%M:%S")
            # Valores que vamos a usar de referencia para compararlos cuando finalize la descarga
            query = f"""UPDATE public."Tanques_tanques" set inicia_entrega = 'True', vol_ref = \'{"{:.2f}".format(float(vol_ref))}\', fecha_ref = \'{fecha}\', vol_ct_ref = \'{"{:.2f}".format(float(vol_ant[3]))}\', agua_ini = \'{"{:.2f}".format(float(vol_ant[4]))}\', temp_ini = \'{"{:.2f}".format(float(vol_ant[5]))}\', vol_ini = \'{"{:.2f}".format(float(volumen))}\' WHERE vr_tanque = \'{vol_act[0]}\'; """
            cur.execute(query)
            conn.commit()
